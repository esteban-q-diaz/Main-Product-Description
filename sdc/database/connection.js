...
const query = {
  ...
  values: [1, 'Colosseum', 549.99, 4.5, 61, 10, 0, 2, 'New/Exclusives', 'Adults Welcome/Buildings/Creator Expert', 'creator-expert', 'https://fecphoto.s3.us-east-2.amazonaws.com/creator_logo_pos_300w.png', 18, 99, 9036, 3575, 0, 'VIP Only! Get a FREE Chariot when you purhcase the exclusive new Colosseum.*', 1, 'https://fecphoto.s3.us-east-2.amazonaws.com/BFCM-live-Promo-Badge-3.png']
}
...
const csvCheckoutImport = {
  text: "COPY productdetails (productName, productPrice, productRating, productReviewCount, productInStock, productBackOrder, productStockLimitations, productExclusiveTags, productSeriesTags, seriesName, seriesImage, productAgeRatingBottom, productAgeRatingTop, productPieceCount, vipPoints, chokeWarning, offersFlyer, signUpOffer, offersImageLink) FROM '/Users/rebrnd/Documents/Hack_Reactor/Immersive/Week_9/sdc/Main-Product-Description/sdc/data_generator/productdetails_pt5.csv' DELIMITER ',' CSV HEADER",
  values: []
 }
...
client.connect()
  ...
  .then(() => client.query("select * from productdetails where productNum = 46"))
  .then((results) => console.table(results.rows))
  // .then(console.timeEnd('checkout'))
  .catch(e => console.log(e))
  .finally(() => console.timeEnd('checkout'))
  .finally(() => client.end())
